...
Un campo de **"e"** (echo) en tipografía que respira al ritmo de algo físico en Lima
—el viento y la hora reales sobre la ciudad— pensado como espejo entre un espacio
físico (un cuarto de servicio intervenido) y la web. Quien entra deja un rastro:
ecos, dibujos o mensajes que les llegan a las demás personas conectadas.

Estética de cableado desordenado, ASCII y ASMR.

- **Campo en vivo** dibujado con [p5.js](https://p5js.org/).
...
1. Crear un repo nuevo en GitHub (ej. `endless-echo`) y subir estos archivos a la
   raíz de la rama `main`:

   ```bash
   git init
   git add .
   git commit -m "endless echo v1"
   git branch -M main
   git remote add origin https://github.com/TU-USUARIO/endless-echo.git
...
5. Esperar ~1 minuto. La pieza queda en:
   `https://TU-USUARIO.github.io/endless-echo/`
